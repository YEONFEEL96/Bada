...
- **Blocking Socket I/O under `withContext(Dispatchers.IO)` does NOT honour coroutine cancellation while parked in a syscall.** `runReceiveLoop` in both connection drivers (and `TcpReceiverServer.stop`) closes the socket BEFORE `cancelAndJoin`'ing the pump, otherwise teardown deadlocks. The same trap applied to the medium providers' server-side `ServerSocket.accept()` (Wi-Fi Direct / hotspot): a `withTimeoutOrNull` around it never fired, so an inbound connection whose peer ignored the upgrade offer hung forever in `Handshaking` (#288). Provider accepts now go through `acceptCancellable()` (closes the listener on cancellation) and the orchestrator closes the listener via `cancelPendingUpgrade()` before joining the accept coroutine.
- **The receiver must keep servicing the LAN channel while its bandwidth-upgrade offer is pending.** Stock GMS senders that ignore or fail an `UPGRADE_PATH_AVAILABLE` keep negotiating (PKE / introduction) on the prior channel; a receiver that parks on the provider's accept until the peer joins the P2P group looks dead and the sender times out before the consent UI ever appears (#288, vivo X200 Pro). `InboundConnectionDriver.runPendingUpgradeLoop` starts the sharing FSM on the bootstrap channel right after the offer, polls that channel + user events + an inline KEEP_ALIVE, and adopts the upgraded transport mid-negotiation when the peer does connect (or abandons the offer on timeout / peer `UPGRADE_FAILURE`). BLE GATT / L2CAP bootstraps keep the legacy complete-upgrade-first ordering (`runBlockingUpgradeThenNegotiate`).
- **Advertise the real Wi-Fi band support in `MediumMetadata`.** A stock Nearby Connections group owner picks the Wi-Fi Direct group's band and channel from the peer's `medium_metadata` (`supports_5_ghz`, `supports_6_ghz`, `ap_frequency`) on the opening `ConnectionRequest` / `UPGRADE_PATH_REQUEST`, plus the `sta_frequency` hint on `SAFE_TO_CLOSE_PRIOR_CHANNEL`. The all-false placeholder from #132 read as "2.4 GHz only", so a Samsung receiver forming the group for a Bada sender pinned it to 2.4 GHz at a tenth of the reverse direction's speed (#287, Xiaomi 15 to Tab S9). `AndroidWifiCapabilities.read` fills `LocalWifiCapabilities` from `WifiManager` and the sender threads it into every metadata site; `:core-protocol` only sees the data class.
